<template>
  <div class="container">
      ...
      <div class="filter">
        <b-dropdown text="Urutkan">
          <b-dropdown-item
            value="1"
            class="primary_hover"
            @click="listenToSort(1)"
            >Semua</b-dropdown-item
          >
          <b-dropdown-item
            value="2"
            class="primary_hover"
            @click="listenToSort(2)"
            >Termahal</b-dropdown-item
          >
          <b-dropdown-item
            value="3"
            class="primary_hover"
            @click="listenToSort(3)"
            >Termurah</b-dropdown-item
          >
        </b-dropdown>
      </div>
    </div>
    <div class="product_container">
      <div
        class="boxs"
        v-for="(item, index) in dataProductList"
        :key="index"
          ...
          productdata.forEach((item) => {
            this.dataProductList.push({
              ...
              category: item.category.name
                ? this.limitText(item.category.name, 15)
                : "no category",
    ...
    listenToSort(data) {
      switch (data) {
        case 1:
          ...
          break;
        case 2:
          this.dataProductList = [...this.temp].sort(
            (a, b) => b.price - a.price
          );
          break;
        default:
